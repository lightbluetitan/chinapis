...
#'
#' @export
get_china_life_expectancy <- function() {

  url <- "https://api.worldbank.org/v2/country/CHN/indicator/SP.DYN.LE00.IN?format=json&date=2010:2022&per_page=100"
  ...
  txt <- tryCatch(
    ...
    error = function(e) {
      message("Failed to retrieve content from response.")
      return(NULL)
    }
  )
  ...
  content <- tryCatch(
    ...
    error = function(e) {
      message("Failed to parse JSON from World Bank API.")
      return(NULL)
    }
  )
